...
// TTS
let useTextToSpeech: boolean = process.env.USE_TTS === "true";
let ttsVoiceID: string = process.env.ELEVEN_LABS_VOICE_ID;
...
let timeBetweenComments: number;
let allowComment: boolean = true;
...
io.on("connection", (socket) => {
  ...
  socket.on("SetStyle", (data: string) => {
    style = data;
    console.log(`Current style: ${style}`);
  });

  // Recieve setting: Time between comments
  socket.on("TimeBetweenComments", (data: number) => {
...
function handleUsername(incomingUsername: string) {
...
function handleComment(user: string, comment: string) {
  if (!allowComment) {
...
async function handleAnswer(question: string) {
  ...
  const result = await callGPT(question, style);
